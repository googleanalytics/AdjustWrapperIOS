...
#import "TestGoogleAnalyticsAdapter.h"
...
- (void)setUp {
  ...
  self.adapter =
      [[GoogleAnalyticsAdapter alloc] initWithSdkAdapter:[[TestGoogleAnalyticsAdapter alloc] init]];
}
...
- (void)testLogEventWithNilName {
  ...
  [self verifyEventWithName:@"test_unnamed_event" parameters:nil];
}
...
- (void)testLogEventWithMappedName {
  [self.adapter logEventWithName:@"mapped_event" parameters:nil];
  [self verifyEventWithName:@"test_event" parameters:nil];
}

- (void)testLogEventWithMappedParameter {
  [self.adapter logEventWithName:@"event" parameters:@{@"mapped_parameter" : @"val"}];
  [self verifyEventWithName:@"event" parameters:@{@"test_parameter" : @"val"}];
}

- (void)testLogEventWithSetApiWrapper {
  [self.adapter logEventWithName:@"event" parameters:@{@"api_wrapper" : @"other_val"}];
  [self verifyEventWithName:@"event" parameters:@{@"test_api_wrapper" : @"other_val"}];
}
  ...
  OCMVerify([self.analyticsMock logEventWithName:@"event"
                                      parameters:[OCMArg checkWithBlock:^BOOL(id obj) {
                                        ...
                                        return parameters.count == 25 &&
                                               [parameters[@"api_wrapper"] isEqual:@"test"];
                                      }]]);
}
...
- (void)testSanitizeName {
  ...
  sanitizedName = [self.adapter sanitizeName:@"_name"
                                ...
                                defaultValue:nil
                               reservedNames:@[]];
  XCTAssertEqualObjects(sanitizedName, @"test__name");
}
...
- (void)testSanitizeRestrictedPrefix {
  NSString *sanitizedName = [self.adapter sanitizeName:@"ga_name"
                                          ...
                                          defaultValue:nil
                                         reservedNames:@[]];
  XCTAssertEqualObjects(sanitizedName, @"test_ga_name");

  sanitizedName = [self.adapter sanitizeName:@"firebase_name"
                                   maxLength:100
                                defaultValue:nil
                               reservedNames:@[]];
  XCTAssertEqualObjects(sanitizedName, @"test_firebase_name");

  sanitizedName = [self.adapter sanitizeName:@"google_name"
                                   maxLength:100
                                defaultValue:nil
                               reservedNames:@[]];
  XCTAssertEqualObjects(sanitizedName, @"test_google_name");
...
- (void)testSanitizeNameWithTruncation {
  ...
  sanitizedName = [self.adapter sanitizeName:@"ga_name"
                                ...
                                defaultValue:nil
                               reservedNames:@[]];
  XCTAssertEqualObjects(sanitizedName, @"test_ga");
}
...
- (void)testSanitizeNameWithInvalidCharacter {
  ...
  XCTAssertEqualObjects(sanitizedName, @"test__a");
  ...
  sanitizedName = [self.adapter sanitizeName:@"!_a!"
                                ...
                                defaultValue:nil
                               reservedNames:@[]];
  XCTAssertEqualObjects(sanitizedName, @"test___a_");
}
...
- (void)testSanitizeNameWithReservedNames {
  NSString *sanitizedName = [self.adapter sanitizeName:@"name"
                                          ...
                                          defaultValue:nil
                                         reservedNames:@[ @"name" ]];
  XCTAssertEqualObjects(sanitizedName, @"test_name");
...
- (void)verifyEventWithName:(NSString *)name parameters:(NSDictionary<NSString *, id> *)parameters {
  ...
  allParameters[@"api_wrapper"] = @"test";
  NSLog(@"Verify %@: %@", name, allParameters);
