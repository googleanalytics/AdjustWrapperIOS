...
@interface TestGoogleAnalyticsAdapter ()
...
@implementation TestGoogleAnalyticsAdapter

- (instancetype)init {
  self = [super init];
  if (self) {
    _sanitizedNamePrefix = @"test_";
    _wrapperParameterValue = @"test";
    _emptyEventName = @"test_unnamed_event";
    _emptyParameterName = @"test_unnamed_parameter";
    _emptyUserPropertyName = @"test_unnamed_up";

    _eventMap = @{@"mapped_event" : @"test_event"};

    _parameterMap = @{@"mapped_parameter" : @"test_parameter"};
  }
